...
export function ProductSetup({kind,onCreate,onCancel}:{kind:'new'|'existing';onCreate:(p:Product)=>void;onCancel:()=>void}){
  ...
  return <form className="setup-form" onSubmit={e=>{e.preventDefault();if(!p.name.trim()||!p.markets.length||!p.areas.length||!p.users.length||!p.benefits.length||!p.channels.length){setError('Enter a name and choose at least one option in each selection.');return;}onCreate(recordChange({...p,name:p.name.trim()},'Created Product Passport and original design brief.',{}));}}>
    ...
    <MarketSelect value={p.markets} onChange={v=>patch({markets:v})}/>{kind==='existing'&&<MarketSelect label="Current markets" value={p.current} onChange={v=>patch({current:v})}/>}
